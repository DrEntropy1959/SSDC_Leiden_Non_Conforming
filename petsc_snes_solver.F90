...
#include "finclude/petscviewer.h"
  ...
  !  Residual Jacobiand and preconditioning matrices
  Mat :: petsc_jacobian_mat, petsc_pc_mat
...
contains
  ...
  subroutine setup_petsc_solver()
    ...
    integer :: min_column_on_proces, max_column_on_proces
    ...
    ! Option 1: let PETSc compute the Jacobian matrix with a finite differencing 
    ! approach without taking advantage of the sparsity. This approach is very 
    ! very slow and expensive.
    ! --------------------------------------------------------------------
    ! call SNESSetJacobian(petsc_snes,petsc_jacobian_mat,petsc_jacobian_mat, &
    !                    & SNESComputeJacobianDefault,PETSC_NULL_OBJECT,i_err)
    ! call check_petsc_error(i_err)


    ! Option 2: provide the function for computing the Jacobian matrix
    ! ----------------------------------------------------------------
    ...
    max_column_on_proces = max_column_on_proces - 1

    ! Count number of nonzeros block element off-process
    do i_brow = 1, size(iaS)-1
      do j_bcolumn = iaS(i_brow), iaS(i_brow+1) - 1
      if((jaS(j_bcolumn) .lt. min_column_on_proces) .or. &
        & (jaS(j_bcolumn) .gt. max_column_on_proces)) o_nnz(i_brow) = o_nnz(i_brow) + 1
      enddo
      d_nnz(i_brow) = (iaS(i_brow+1) - iaS(i_brow)) - o_nnz(i_brow)
    enddo
    ...
    ! Residual Jacobian matrix is equal to preconditioning matrix
    ...
    max_iters_ksp = 200
    call KSPSetTolerances(petsc_ksp,PETSC_DEFAULT_REAL, &
      & PETSC_DEFAULT_REAL,d_tol_ksp,max_iters_ksp,i_err)
    call check_petsc_error(i_err)
  ...
  !============================================================================
  !
  ! form_jacobian_matrix - Forms the analytical residual Jacobian matrix, i.e.
  ! dRHS/dCons.
  !
  ! Input parameters:
  ...
  !
  ! Output parameter:
  ! jacobian_mat - residual Jacobian matrix.
  ! pc_mat - preconditioning matrix.
  ! flag - flag indicating matrix structure. 

  subroutine form_jacobian_matrix(snes,x_vec,jacobian_mat,pc_mat,flag,dummy, &
      & i_err)
    ...
    use CSRlocalvariables
    use jacobian_matrix_implicit_ts, only : communicate_data_jacobian, &
      & compute_residual_jacobian_element
    ...
    SNES, intent(in) :: snes
    Vec, intent(in) :: x_vec
    Mat, intent(out) :: jacobian_mat, pc_mat
    ...
    stage_update_pc_mat = 2

    !if (current_stage_imex .eq. stage_update_pc_mat) then

      !write(*,*) 'Computing Jacobian'
      ...
      !Initialize elems_counter
      elems_counter = 1
      
      ! Communicate data across the parallel interfaces before computing the
      ! Jacobian matrix
      call communicate_data_jacobian()
...
!       if(i_elem == 2) then
!         write(*,*)'inside',maxval(abs(tmp_save(:,:,:)-dfdu_a_elem(:,:,52:60)))
!       endif
        ...
        do i_node = 1, nodesperelem
          ...
          !write(*,*) 'nodes_counter', nodes_counter
          !write(*,*) 'nnz_blocks', nnz_blocks_per_block_row
          !write(*,*) 'shift', shift
        ...
        ! Update elems_counter
        elems_counter = elems_counter + 1

      enddo

      ! Same nonzero pattern for preconditioning 
      ...
      if(jacobian_mat .ne. pc_mat ) then
        call MatAssemblyBegin(jacobian_mat,MAT_FINAL_ASSEMBLY,i_err)
        call check_petsc_error(i_err)
        call MatAssemblyEnd(jacobian_mat,MAT_FINAL_ASSEMBLY,i_err)
        call check_petsc_error(i_err)
      endif
  ...
  subroutine solve_implicit_petsc(converged)
    ...
    ! Recompute preconditioning matrix
    ! Since in form_jacobi_matrix() there is an if statement that checks if the
    ! current_stage_imex is equal to to this will only have the effect to
    ! compute the preconditioner with the final solution at stage 2.
    call form_jacobian_matrix(petsc_snes,petsc_x_vec,petsc_jacobian_mat, &
      & petsc_pc_mat,flag,dummy,i_err)
  ...
  subroutine finalize_petsc_solver()
    ...
    call MatDestroy(petsc_jacobian_mat,i_err)
    call check_petsc_error(i_err)
