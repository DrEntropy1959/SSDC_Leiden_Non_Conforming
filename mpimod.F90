  ...
  subroutine distribute_elements_aflr3()
    ...
    if (myprocid == 0) then
      ...
      do i_proc = 0, nprocs-1
        ...
        allocate(vert_old_2_new(max_n_vert_per_proc))
...
!      write(889,*) tmp_ldg_flip_flop_sign_tmp2
  ...
  subroutine PetscGridLocations_LGL()
    ...
    do ielem = ihelems(1), ihelems(2)
      ...
      nelem_ghst(1,ielem) = nghost
      nelem_ghst(2,ielem) = nghost_NonCon
...
!      do iface = 1, nfacesperelem
  ...
  subroutine PetscNormals_LGL()
    ...
    integer :: ielem, iface, n_S_2d_Off
    ...
    do ielem = ihelems(1), ihelems(2)
      ...
      do iface = 1, nfacesperelem
        ...
        if(ef2e(3,iface,ielem) == myprocid) then
          ...
          call element_properties(ef2e(2,iface,ielem), n_pts_2d=n_S_2d_Off)

          nghost_LGL_shell = nghost_LGL_shell + n_S_2d_Off
    ...
    call UpdateComm1D_LGL_Shell_GhostData(Jx_facenodenormal_LGL,nxghst_LGL_shell,nxpetsc_shell,nxlocpetsc_shell, &
...
!      face_loop:do iface = 1,nfacesperelem               ! loop over faces on element
      face_loop:do iface = 1,2*ndim                       ! loop over faces on element HACK
        ...
        do i = 1, nodesperface                           ! loop over nodes on neighbor face
            ...
            iyu(iloc) = nk *(kelem-1)           &        ! stride through padded element blocks
                      + kfacenodes(i,kface)              ! sweep over face planes of data
  ...
  end subroutine PetscComm0DDataSetup

  !============================================================================

  subroutine PetscComm1DDataSetup(Zin, Zghstin, Zpetscin, Zlocin, nq, nk, ne, ngh)
...
! !============================================================================
  ...
  subroutine PetscComm1D_LGL_Shell_DataSetup(Zin, Zghstin, Zpetscin, Zlocin, nq, nk, ne, ngh)
    ...
    do ielem = ihelems(1), ihelems(2)                ! loop over elements
      ...
      do iface = 1,nfacesperelem                     ! loop over faces on elem
        ...
        if(ef2e(3,iface,ielem) == myprocid) then
          ...
          kelem = ef2e(2,iface,ielem)                ! element of neighbor
          kface = ef2e(1,iface,ielem)                ! element of neighbor

          call element_properties(kelem, n_pts_2d=n_S_2d_Off) ! size is element dependent
  
          do i = 1, n_S_2d_Off                       ! loop over nodes on neighbor face
  ...
  end subroutine PetscComm1D_Gau_Mortar_DataSetup
  ...
  subroutine UpdateComm0DGhostData(Zin, Zghstin, Zpetscin, Zlocin, nk, ngh)
    ...
    ntotu = nk * nelems       ! length of on process data
    ntotw = nk                ! length of incoming block of data WITH PADDING of zeros

    do ielem = ihelems(1), ihelems(2)                       ! loop over elements

      call element_properties(ielem, n_pts_3d=nodesperelem) ! size is element dependent

      ntot = nodesperelem                                   ! bucket size
      ...
      do inode = 1, nodesperelem                            ! loop over nodes
           ...
           yu(inode) = Zin(inode,ielem)                     ! update temporary solution values
                                                            ! update global location of solution values
          iyu(inode) = ntotw*(ielem-1)    &                 ! shift over previous elements
                     +        inode       &                 ! shift over previous nodes
...
!      do iface = 1,nfacesperelem                            ! loop over faces
  ...
  end subroutine UpdateComm0DGhostData
...
!   integer :: ntotu, ntotv, ntotw, ntot
  ...
  subroutine UpdateComm1D_LGL_Shell_GhostData(Zin, Zghstin, Zpetsc, Zlocpetsc, nq, nk, ngh)
    ...
    integer :: kelem, n_S_2d_On, n_S_2d_Off
    integer :: i, ieq, jnode
    integer,  allocatable :: iyu(:)
    real(wp), allocatable ::  yu(:)
    ...
    do ielem = ihelems(1), ihelems(2)                       ! loop over elements
      ...
      do iface = 1, nfacesperelem                           ! loop over 6 faces
        ...
        if(ef2e(3,iface,ielem) == myprocid) then
          ...
          call element_properties(ielem, n_pts_2d=n_S_2d_On) ! size is element dependent
          ...
          do i = 1, n_S_2d_On                                 ! loop over nodes on face
 
            jnode = (iface-1)*n_S_2D_On + i                   ! access shell coordinate
    ...
    do ielem = ihelems(1), ihelems(2)                       ! loop over elements
      ...
      do iface = 1,nfacesperelem                            ! loop over faces
        ...
        if(ef2e(3,iface,ielem) == myprocid) then
          ...
          call element_properties(kelem, n_pts_2d=n_S_2d_Off)

          do i = 1, n_S_2d_Off                               ! loop over nodes
  ...
  subroutine UpdateComm1D_Gau_Mortar_GhostData(Zin, Zghstin, Zpetsc, Zlocpetsc, nq, nk, ngh)
...
!==================================================================================================
 ...
 character(len=1024)                                :: file_name_loc 
...
!  call MPI_FILE_CLOSE(thefile, ierr)
    ...
    call get_unit(iunit)
     ...
     write(numb,'(I0)')myprocid
     file_name_loc = 'proc_'//trim(adjustl(numb))//file_name
    write(*,*)'myprocid = ',myprocid,' writing to iunit = ',iunit,&
      ' file = ',file_name_loc

    !-- open the file   
    open(UNIT=iunit,FILE=file_name_loc,STATUS='REPLACE',FORM='FORMATTED',IOSTAT=ios)
    if(ios.NE.0) then 
      write(*,*)"File = ",file_name_loc," not opened correctly in initgrd: write_element_to_file(), iostat = ",ios
      stop
    endif

    !-- write the nodal information to file
    do ielem = ihelems(1), ihelems(2)
    ...
    !-- open the file
    open(UNIT=iunit,FILE='surf'//file_name_loc,STATUS='REPLACE',FORM='FORMATTED',IOSTAT=ios)
    if(ios.NE.0) then 
      write(*,*)"File = ",'surf'//file_name_loc," not opened correctly in initgrd: write_element_to_file(), iostat = ",ios
      stop
    endif
    
    !-- write the mortar nodal locations to file
    do ielem = ihelems(1), ihelems(2)
      !-- obtain element properties
      write(numb,'(I0)')ielem
      call element_properties(ielem,n_pts_3d=n_pts_3d)
    ...
    !-- close file
    close(UNIT=iunit)
    write(*,*)'Mortar nodes written to ', 'surf'//file_name_loc 

    !-- open the file
    open(UNIT=iunit,FILE='J'//file_name_loc,STATUS='REPLACE',FORM='FORMATTED',IOSTAT=ios)
    if(ios.NE.0) then 
      write(*,*)"File = ",'J'//file_name_loc," not opened correctly in initgrd: write_element_to_file(), iostat = ",ios
      stop
    endif

    !-- write the Jacobian information to file
    do ielem = ihelems(1), ihelems(2)
      !-- obtain element properties
      write(numb,'(I0)')ielem
      call element_properties(ielem,n_pts_3d=n_pts_3d)
    ...
    enddo
       !-- close file
    close(UNIT=iunit)
    write(*,*)'Jacobian written to ', 'J'//file_name_loc

    !-- open the file
    open(UNIT=iunit,FILE='r_x'//file_name_loc,STATUS='REPLACE',FORM='FORMATTED',IOSTAT=ios)
    if(ios.NE.0) then 
      write(*,*)"File = ",'r_x'//file_name," not opened correctly in initgrd: write_element_to_file(), iostat = ",ios
      stop
    endif

    !-- write the r_x information to file
    do ielem = ihelems(1), ihelems(2)
      !-- obtain element properties
      write(numb,'(I0)')ielem
      call element_properties(ielem,n_pts_3d=n_pts_3d)
    ...
    enddo
       !-- close file
    close(UNIT=iunit)
    write(*,*)'Scaled metrics written to ', 'r_x_'//file_name_loc
 
    !-- open the file
    open(UNIT=iunit,FILE='x_r'//file_name,STATUS='REPLACE',FORM='FORMATTED',IOSTAT=ios)
    if(ios.NE.0) then 
      write(*,*)"File = ",'x_r'//file_name," not opened correctly in initgrd: write_element_to_file(), iostat = ",ios
      stop
    endif

    !-- write the x_r information to file
    do ielem = ihelems(1), ihelems(2)
      !-- obtain element properties
      write(numb,'(I0)')ielem
      call element_properties(ielem,n_pts_3d=n_pts_3d)
    ...
    !-- close file
    close(UNIT=iunit)
     write(*,*)'Inverse scaled metrics written to ', 'x_r_'//file_name_loc 

    !-- obtain a free Fortarn unit
    call get_unit(iunit)
    
    !-- open the file
    open(UNIT=iunit,FILE='Jx_facenodenormal_LGL'//file_name_loc,STATUS='REPLACE',FORM='FORMATTED',IOSTAT=ios)
    if(ios.NE.0) then 
      write(*,*)"File = "//'Jx_facenodenormal_LGL'//file_name_loc,&
        " not opened correctly in initgrd: write_element_to_file(), iostat = ",ios
      stop
    endif
    
    !-- write Jx_facenodenormal_LGL information to file
    do ielem = ihelems(1), ihelems(2)
      !-- obtain element properties
      write(numb,'(I0)')ielem
    ...
    enddo
       !-- close file
    close(UNIT=iunit)
    write(*,*)'Jx_facenodenormal_LGL written to','Jx_facenodenormal_LGL'//file_name_loc

end subroutine parallel_write_grid_to_file
